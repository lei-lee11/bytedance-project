...
export class LangGraphStorageAdapter extends BaseCheckpointSaver {
    ...
    try {
      ...
      if (hasToolCalls) {
        ...
        return content.includes("吗？") ||
          content.includes("可以帮助") ||
          content.includes("我可以")
          ? "aimessage"
          : "humanmessage";
      }
    ...
    for (const message of newMessages) {
      try {
        ...
        if (messageType === "humanmessage" || messageType === "human") {
          ...
          const updatedUserHistory = await this.storage.history.getHistory(
            threadId,
            {
  ...
  private checkpointToState(checkpoint: Checkpoint): AgentState {
    ...
    return {
      ...
      taskStatus:
        (values.taskStatus as "planning" | "executing" | "completed") ||
        "planning",
  ...
  async put(
    ...
    try {
      ...
      const userMessages = previousHistory.filter(
        (record) => record.event_type === "user_message",
      );
      const aiMessages = previousHistory.filter(
        (record) => record.event_type === "ai_response",
      );
      ...
      if (state.messages && state.messages.length > 0) {
        await this.saveMessagesToHistory(
          threadId,
          state.messages,
  ...
  async putWrites(
    ...
    try {
      ...
      const latestCheckpoint =
        await this.storage.checkpoints.getLatestCheckpoint(threadId);
      ...
      for (const [channel, value] of writes) {
        if (channel === "messages" && Array.isArray(value)) {
          ...
          const processedMessages = this.applyMessagesReducer(
  ...
  private async saveCheckpointDirectly(
    ...
    if (existingMetadata) {
      // 计算实际的消息数量（去除重复消息）
      const deduplicatedMessages = this.deduplicateMessages(
        state.messages || [],
      );
  ...
  async getTuple(config: RunnableConfig): Promise<CheckpointTuple | undefined> {
    ...
    if (config.configurable?.checkpoint_id) {
      // 获取特定检查点
      checkpoint = await this.storage.checkpoints.getCheckpoint(
        threadId,
    ...
    return {
      ...
      checkpoint: {
        ...
        ts: new Date().toISOString(),
        channel_values: checkpointData.channel_values as unknown as Record<
          string,
          unknown
        >,
        channel_versions: {},
  ...
  async *list(
    ...
    for (let i = checkpoints.length - 1; i >= 0; i--) {
        ...
        try {
          ...
          if (beforeCheckpointId) {
            ...
            const targetCheckpoint =
              await this.storage.checkpoints.getCheckpoint(
                threadId,
                beforeCheckpointId,
              );
            if (targetCheckpoint) {
          ...
          console.warn(
            `比较检查点时间戳时出错 (checkpoint: ${checkpointRecord.checkpoint.id}):`,
            error,
          );
          // 如果比较失败，保守地包含该检查点
        }
      ...
      try {
        ...
        // 跳过损坏的检查点
        console.warn(
          `跳过损坏的检查点: ${checkpointRecord.checkpoint.id}`,
          error,
        );
      }
    }
